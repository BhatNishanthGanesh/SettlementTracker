...
//         const dailySavings = calculateSavings(expenses, parseFloat(dailyBudget));
...
//     const dailySavings = Object.entries(groupedExpenses).map(([date, expensesOfDay]) => {
//       const totalSpent = expensesOfDay.reduce((acc, expense) => acc + expense.spent, 0);
...
//   <li key={index}>
//     {saving.date}: ₹{saving.savings.toFixed(2)} 
//   </li>
// ))}
...
//   const [savings, setSavings] = useState([]);
const [savings, setSavings] = useState<Array<{ date: string; savings: number }>>([]);
...
const calculateSavings = (expenses:any, dailyBudget:any) => {
    ...
    // Calculate savings for each day
    const dailySavings = Object.entries(groupedExpenses).map(([date, expensesOfDay]:any) => {
      const totalSpent = expensesOfDay.reduce((acc:any, expense:any) => acc + expense.spent, 0);
      console.log(' totalSpent:', totalSpent); 
  
      console.log('calculateSavings - inside map - dailyBudget:', dailyBudget);
...
return (
    <div className="container mx-auto mt-8 p-8">
      {showForm ? (
        <form onSubmit={handleFormSubmit} className="max-w-md mx-auto bg-white p-8 rounded shadow-lg">
        ...
        <>
        <h1 className='text-center text-5xl font-mono dark:text-white'>Savings</h1>
        <p className='font-bold dark:text-white mb-2'>Budget for a day: {dailyBudget}</p>
  ...
  <div
    key={index}
    className={`bg-white dark:text-white dark:bg-medium p-4 rounded ${hoveredCard === index ? '' : 'shadow-lg'} transition-shadow duration-300`}
    // @ts-ignore
