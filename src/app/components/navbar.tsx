    ...
    <div className="flex items-center">
      {session ? (
        <div className="flex items-center relative">
          ...
          {showDropdown && (
            <div
              className="absolute  bg-white shadow-md rounded-md mt-16 p-2"
              ref={dropdownRef}
            >
      ...
      <div className="flex-grow ">
        <div className="">
          <div className="lg:hidden">
            <button
              onClick={toggleMenu}
              className="text-black dark:text-white p-3 focus:outline-none"
            >
              {/* Hamburger menu icon */}
              <svg
          ...
          <div className={`lg:flex ${showMenu ? "block" : "hidden"}`}>
            ...
            {Links.map((link, index) => (
              <Link key={index} href={link.href}>
                ...
                >
                  <div
                    className={`flex dark:bg-medium border-4 dark:border-medium border-white rounded px-4 py-2 m-2 items-center ${
                      path === link.href ? "" : "shadow-2xl"
                    }`}
                  >
